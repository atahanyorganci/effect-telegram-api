...
describe("getChatMemberCount", () => {
	describe("success", () => {
		it.effect("returns the member count for a valid chat_id", () =>
			Effect.gen(function* () {
				const count = yield* callGetChatMemberCount(requireBotToken(), { chat_id: requireChatId() });
	...
	describe("Telegram API errors", () => {
		it.effect("ChatNotFound when chat_id does not exist", () =>
			Effect.gen(function* () {
				const error = yield* callGetChatMemberCount(requireBotToken(), { chat_id: 0 }).pipe(Effect.flip);
		...
		it.effect("ChatIdEmpty when chat_id is missing", () =>
			Effect.gen(function* () {
				const error = yield* callGetChatMemberCount(requireBotToken(), {}).pipe(Effect.flip);
