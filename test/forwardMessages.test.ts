...
const callForwardMessages = (token: string, payload: unknown) => callClient("forwardMessages", token, payload as never);

liveTests("forwardMessages", test => {
	describe("success", () => {
		test.effect("returns forwarded message ids", () =>
			Effect.gen(function* () {
	...
	authErrorTests(test, token => callForwardMessages(token, { chat_id: 1, from_chat_id: 1, message_ids: [1] }));
});
