...
describe("getChatAdministrators", () => {
	describe("Telegram API errors", () => {
		it.effect("ChatNotFound when chat_id does not exist", () =>
			Effect.gen(function* () {
				const error = yield* callGetChatAdministrators(requireBotToken(), { chat_id: 0 }).pipe(Effect.flip);
		...
		it.effect("ChatIdEmpty when chat_id is missing", () =>
			Effect.gen(function* () {
				const error = yield* callGetChatAdministrators(requireBotToken(), {}).pipe(Effect.flip);
