...
describe("getUserProfileAudios", () => {
	...
	describe("Telegram API errors", () => {
		it.effect("InvalidUserId when user_id is missing", () =>
			Effect.gen(function* () {
				const error = yield* callGetUserProfileAudios(requireBotToken(), {}).pipe(Effect.flip);
		...
		it.effect("UserNotFound when user_id does not exist", () =>
			Effect.gen(function* () {
				const error = yield* callGetUserProfileAudios(requireBotToken(), { user_id: 999_999_999_999 }).pipe(
					Effect.flip,
				);
